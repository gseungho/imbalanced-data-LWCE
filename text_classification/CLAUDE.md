...
## 실험 결과 (11종, 5 seeds, 2026-07 재실행 — gradient/per-class 포함)

데이터: **IR=328.8:1** (min=39 `grief`, max=12823 `neutral`, 총 36,308 / val 4,548 / test 4,590, vocab 12,226).
Optuna best: `pwce α=0.46`, `plwce α=4.10`, `focal γ=2.52`, `eslwce ε=1.74`, `logitadj τ=0.31`,
`combined α=4.17·ε=0.10`. **proposed = lwce/plwce/eslwce**, combined는 §4.5 ablation, logitadj는 §4.2 baseline.

| Loss | F1-Macro | Top1 | Balanced | Worst | G-Mean | Few |
|------|----------|------|----------|-------|--------|-----|
| ce | 0.4123±0.0098 | 0.5449 | 0.3997 | 0.0000 | 0.0729 | 0.3260 |
| wce | 0.3735±0.0088 | 0.4220 | 0.4161 | 0.0270 | 0.1877 | 0.4081 |
| pwce | 0.4231±0.0150 | 0.5122 | 0.4473 | 0.0247 | 0.1517 | 0.4128 |
| sqce | 0.4290±0.0110 | 0.5220 | 0.4569 | 0.0237 | 0.1719 | 0.4281 |
| **lwce** | 0.4190±0.0060 | 0.5363 | 0.4165 | 0.0000 | 0.0936 | 0.3605 |
| **plwce** | **0.4343±0.0123 (1위)** | 0.5431 | 0.4664 | 0.0488 | 0.2402 | 0.4685 |
| **eslwce** | 0.4192±0.0065 | 0.5346 | 0.4133 | 0.0000 | 0.1109 | 0.3513 |
| combined | 0.4342±0.0122 | 0.5427 | 0.4664 | 0.0488 | 0.2402 | 0.4685 |
| cb | 0.3988±0.0110 | 0.4523 | 0.4691 | **0.0801** | **0.3219** | **0.4940** |
| focal | 0.4049±0.0015 | 0.5444 | 0.3904 | 0.0000 | 0.0343 | 0.3074 |
| logitadj | 0.4214±0.0052 | 0.5198 | 0.4317 | 0.0000 | 0.0910 | 0.3745 |

Gradient ratio(Few/Many, 마지막 epoch): wce **5.21** > cb 4.74 > combined 3.78 ≈ plwce 3.75 > sqce 3.64 > pwce 3.35 > **lwce 2.41 ≈ eslwce 2.20 ≈ ce 2.14** > focal 1.94.

### 주요 소견

- **plwce가 F1-Macro 단독 1위(0.4343)**. vs ce: **Few +0.143(0.326→0.469), Top1 −0.002(무손실)**.
- **⚠️ combined ≈ plwce로 완전 환원**: Optuna가 combined의 ε를 그리드 하한(0.10)으로 골라
  `combined(α=4.17,ε=0.1) ≈ plwce(α=4.10)`. F1·Worst·G-Mean·Few 소수점까지 동일.
  **논문 §4.5에 "combined는 plwce와 통계적으로 구분 불가 — ε가 정확도 knob이 아님을 실증"으로 정직하게 서술.**
- **Worst-class는 28-class라 변별력 없음**: F1 상위 손실(plwce 제외) 다수가 **Worst=0**(한 클래스라도 recall 0).
  → **G-Mean으로 보고**: plwce/combined **0.2402** vs ce 0.073, focal 0.034. **논문은 Worst 대신 G-Mean을 전면에.**
- **cb/wce의 높은 Few·G-Mean은 head 붕괴의 대가**: cb Few 0.494·G-Mean 0.322 최고지만 Top1 **−0.093**·F1 하위.
  wce Few 0.408이지만 Top1 **−0.123**·F1 **꼴찌**. gradient ratio도 wce 5.21로 최대(explosion).
  **plwce는 cb 다음가는 G-Mean(0.240)을 Top1 손실 없이 달성** → Proposition 3·4 직접 실증.
- **plwce(튜닝 α)·combined 강세 / lwce·eslwce(parameter-free 계열) 보수적** — CIFAR-100(다클래스) 패턴이
  **텍스트+트랜스포머에서 재현** → 아키텍처 독립성 주장 뒷받침.
- **eslwce ≈ lwce**(F1 0.4192 vs 0.4190): 예상대로 ES-LWCE는 LWCE 근방(ε가 완화 방향으로만 작동).
